...
module.exports = ()=> {
    ...
    u.debug = function(s) {
        ...
        var str = `(node) util.debug is deprecated. Use console.error instead.\nDEBUG: ${s}`;
        console.error(str);
    };

    u.error = function(s) {
        var str = `(node) util.error is deprecated. Use console.error instead.\nDEBUG: ${s}`;
        console.error(str);
    };

    var debugs = {};
    var debugEnviron;
    u.debuglog = function(set) {
        ...
        if (!debugs[set]) {
            if (new RegExp('\\b' + set + '\\b', 'i').test(debugEnviron)) {
                var pid = process.pid;
                debugs[set] = function() {
                    var msg = u.format.apply(u, arguments);
    ...
    u.deprecate = function(func, string) {
    ...
    u.inherits = function(ctor, superCtor) {
...
// Don't use 'blue' not visible on cmd.exe
    ...
    u.log = function() {
        console.log('%s - %s', timestamp(), u.format.apply(u, arguments));
    };

    u.print = u.deprecate(function() {
        var s = '';
    ...
    u.pump = u.deprecate(function() {
    }, 'util.pump is deprecated. Use stream.pipe instead.');
    ...
    function inspectPromise(p) {
        ...
        if (p.toString().indexOf('Promise') === -1)                                                     //TODO change
            return null;
    ...
    u.isNullOrUndefined = isNullOrUndefined;
